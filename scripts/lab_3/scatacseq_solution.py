...
adata = ad.io.read_h5ad(DATA_DIR / DATASET_ID / "processed" / "adata_filtered_vartiles.h5ad")
adata
...
# %%
# Implement your solution here
downsampled_data = np.random.choice(adata.X.data, size=10000, replace=False)
downsampled_data = downsampled_data.astype(int)

fig, ax = plt.subplots(figsize=(6, 4))
sns.histplot(downsampled_data, ax=ax)
ax.set(xlim=(-0.1, downsampled_data.max() + 1.5), xticks=np.arange(downsampled_data.max() + 1.5))
plt.show()

print(f"Largest count value in data: {adata.X.max()}")
...
#   <blockquote>81.45%</blockquote>
# </div>

# %%
# Implement your solution here
adata.obs["n_fragments"] = adata.X.sum(axis=1).A1
adata.obs["log1p_n_fragments"] = np.log1p(adata.obs["n_fragments"])

adata.var["n_fragments"] = adata.X.sum(axis=0).A1
adata.var["log1p_n_fragments"] = np.log1p(adata.var["n_fragments"])

percentage = ((adata.X > 0).sum(axis=0).A1 >= 50).sum() / adata.n_vars * 100
print(f"Percentage of tiles accessible in at least 50 cells: {percentage:.2f}%.")
del percentage
...
fragments = pd.read_csv(
    DATA_DIR / DATASET_ID / "processed" / "fragments.tsv.gz",
    delimiter="\t",
    header=None,
    names=["chr", "start", "end", "barcode", "read_support"],
    nrows=50000,
)
fragments.head()
...
#   <blockquote>138.52812</blockquote>
# </div>

# %%
# Implement your solution here
fragments["length"] = fragments["end"] - fragments["start"]
print(f"Mean fragment length: {fragments['length'].mean()}")

# %%
# Implement your solution here
fig, ax = plt.subplots(figsize=(6, 4))
sns.kdeplot(data=fragments, x="length", color="black", ax=ax)
ymin, ymax = ax.get_ylim()
ax.vlines(x=np.arange(start=147, stop=147 * 10, step=147), ymin=ymin, ymax=ymax, color="#A5A5A5", linestyles="--")
ax.set(xlabel="Fragment length", ylim=[ymin, ymax])
plt.show()
...
# Reading directly with pybedtools does not work on the cluster
# peaks = BedTool(DATA_DIR / DATASET_ID / "processed" / "peaks.bed")

# Work around on the cluster
peaks = pd.read_csv(DATA_DIR / DATASET_ID / "processed" / "peaks.bed", sep="\t")
...
#   <blockquote>88.84%</blockquote>
# </div>

# %%
# Implement your solution here
tiles_bed = BedTool.from_dataframe(adata.var[["chr", "start", "end"]])
tile_peak_overlaps = tiles_bed.intersect(peaks, wa=True, wb=True)
tile_peak_overlaps = tile_peak_overlaps.to_dataframe(
    names=["tile_chr", "tile_start", "tile_end", "peak_chr", "peak_start", "peak_end"]
)
tile_peak_overlaps.head()

# %%
# Implement your solution here
overlapping_tiles = tile_peak_overlaps.loc[:, tile_peak_overlaps.columns.str.startswith("tile_")].drop_duplicates()
overlapping_tiles = (overlapping_tiles["tile_chr"] + "_" + overlapping_tiles["tile_start"].astype(str)).values
adata.var["overlaps_peak"] = adata.var_names.isin(overlapping_tiles)

print(f"Percentage of tiles overlapping with peaks: {adata.var['overlaps_peak'].sum() / adata.n_vars * 100:.2f}")
...
# %%
# Implement your solution here
adata.obs["frip"] = adata[:, adata.var["overlaps_peak"]].X.sum(axis=1).A1 / adata.obs["n_fragments"].values

print(f"Mean FRiP: {adata.obs['frip'].mean():.4f}")
...
sns.histplot(data=adata.obs, x="log1p_n_fragments", bins=50, ax=ax[0])
ax[0].set(title="log1p(fragments per cell)")
sns.histplot(data=adata.var, x="log1p_n_fragments", bins=50, ax=ax[1])
ax[1].set(title="log1p(fragments per tile)")
sns.histplot(data=adata.obs, x="frip", bins=50, ax=ax[2])
ax[2].set(title="FRiP")
...
#     <blockquote>chr10_117658000</blockquote>
...
adata.X = csr_matrix(adata.X / adata.obs[["n_fragments"]].values)

adata.var["idf"] = adata.n_obs / adata.var["n_fragments"]
adata.var["log_idf"] = np.log(adata.var["idf"])
...
# %%
# Implement your solution here
u, s, vh = svds(adata.X, k=30)
adata.obsm["lsi"] = u * s[None, :]
...
# %%
# Implement your solution here
sc.pp.neighbors(adata=adata, n_pcs=30, use_rep="lsi")
sc.tl.umap(adata=adata)

fig = sc.pl.embedding(adata, basis="umap", color="log1p_n_fragments", return_fig=True)
...
# %%
# Implement your solution here
fig = sc.pl.embedding(adata, basis="umap", return_fig=True, s=20)

ax = fig.get_axes()
ax[0].scatter(*adata.obsm["X_umap"][adata.obs["metacell_center"], :].T, s=20, c="black")
ax[0].set(title="Metacell centers")
...
# Pandas based
# metacell_adata = adata.to_df(layer="counts")
# metacell_adata["metacell_assignment"] = adata.obs["metacell_assignment"]
# metacell_adata = metacell_adata.groupby("metacell_assignment", observed=False).sum()

# metacell_adata = ad.AnnData(metacell_adata)
# metacell_adata.X = csr_matrix(metacell_adata.X)
# metacell_adata

metacell_adata = sc.get.aggregate(adata, by="metacell_assignment", layer="counts", func="sum")
metacell_adata.X = csr_matrix(metacell_adata.layers.pop("sum"))
metacell_adata.var = metacell_adata.var[["chr", "start", "end"]]
metacell_adata
...
# %%
# Implement your solution here
metacell_adata.obs["n_fragments"] = metacell_adata.X.sum(axis=1).A1
metacell_adata.obs["log1p_n_fragments"] = np.log1p(metacell_adata.obs["n_fragments"])

fig, ax = plt.subplots(ncols=2, figsize=(12, 4))
sns.histplot(data=adata.obs, x="log1p_n_fragments", bins=50, ax=ax[0])
ax[0].set(title="log1p(fragments per cell)")
sns.histplot(data=metacell_adata.obs, x="log1p_n_fragments", bins=50, ax=ax[1])
ax[1].set(title="log1p(fragments per metacell)")
plt.show()

print(
    f"Average fold change metacell vs. sc: {metacell_adata.obs['n_fragments'].mean() / adata.obs['n_fragments'].mean():.0f}"
)
...
# %%
# Implement your solution here
metacell_adata.layers["counts"] = metacell_adata.X.copy()

metacell_tfidf = adata.to_df()
metacell_tfidf["metacell_assignment"] = adata.obs["metacell_assignment"]
metacell_tfidf = metacell_tfidf.groupby("metacell_assignment", observed=False).sum()

metacell_adata.X = metacell_tfidf.loc[metacell_adata.obs_names, metacell_adata.var_names].values
metacell_adata.X = csr_matrix(metacell_adata.X)
del metacell_tfidf

metacell_gene_score = adata.obsm["gene_score"].copy()
metacell_gene_score["metacell_assignment"] = adata.obs["metacell_assignment"]
metacell_gene_score = metacell_gene_score.groupby("metacell_assignment", observed=False).sum()
metacell_adata.obsm["gene_score"] = metacell_gene_score.loc[metacell_adata.obs_names, :]
del metacell_gene_score
...
# %%
# Implement your solution here
u, s, vh = svds(metacell_adata.X, k=30)
metacell_adata.obsm["lsi"] = u * s[None, :]

sc.pp.neighbors(adata=metacell_adata, n_pcs=30, use_rep="lsi")
sc.tl.umap(adata=metacell_adata)
...
sc.pl.embedding(metacell_adata, basis="umap", color=markers, vmin="p1", vmax="p99", s=50, add_outline=1)
plt.show()
