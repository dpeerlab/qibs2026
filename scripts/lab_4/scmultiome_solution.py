...
from scipy.stats import levene, pearsonr, spearmanr, ttest_ind
...
mdata = md.read_h5mu(DATA_DIR / DATASET_ID / "processed" / "mdata.h5mu")
mdata
...
data = (
    mdata["rna"]
    .obs[["total_counts"]]
    .merge(mdata["atac"].obs[["n_fragments", "frip"]], left_index=True, right_index=True)
)
data = np.log1p(data)

fig, ax = plt.subplots(ncols=2, figsize=(12, 6), sharey=True)

corr = spearmanr(data["total_counts"], data["n_fragments"])
sns.scatterplot(data, x="n_fragments", y="total_counts", c="#999999", ax=ax[0])
ax[0].set(xlabel="Number of fragments", ylabel="Library size", title=f"Spearman correlation: {corr.statistic:.4f}")

sns.scatterplot(data, x="frip", y="total_counts", c="#999999", ax=ax[1])
corr = spearmanr(data["total_counts"], data["frip"])
ax[1].set(xlabel="FRiP", title=f"Spearman correlation: {corr.statistic:.4f}")
...
#     <blockquote>6346bp</blockquote>
#     <li>What is the furthest distance between a peak and its nearest TSS?</li>
#     <blockquote>1,107,269bp</blockquote>
#     <li>What is the unit of these distances?</li>
#     <blockquote>Basepairs</blockquote>
#   </ol>
# </div>

# %%
# Implement your solution here
print(f"Median distance between peaks and their nearest TSSs: {mdata['atac'].var['dist_to_tss'].median()} basepairs")
print(f"Maximum distance between a peak and its nearest TSS: {mdata['atac'].var['dist_to_tss'].max()} basepairs")
...
# %%
# Implement your solution here
var_mask = mdata["atac"].var["dist_to_tss"] < 50_000
var_mask = var_mask & mdata["atac"].var["nearest_gene"].isin(mdata["rna"].var_names)
peak_gene_pairs = mdata["atac"].var.loc[var_mask, ["nearest_gene"]].reset_index(names="peak")

var_mask = var_mask & mdata["atac"].var["peak_type"].isin(["Promoter"])
promoter_gene_pair = mdata["atac"].var.loc[var_mask, ["nearest_gene"]].reset_index(names="peak")

print(
    "Average distance between promoter-annotated peaks and their TSS: "
    f"{mdata['atac'].var.loc[promoter_gene_pair['peak'].values, 'dist_to_tss'].mean():.0f}bp"
)

promoter_gene_pair.head()
...
#   <blockquote>0.0118</blockquote>
# </div>

# %%
# Implement your solution here

# Efficient implementation to include for covariance structure
# corr is a 2n x 2n array, where n denotes the number of promoter-gene pairs
# The top left n x n subarray contains the column-wise correlations of the first argument (atac here)
# The bottom right n x n subarray contains the column-wise correlations of the second argument (rna here)
# The top right and bottom left n x n subarray contains the column-wise correlations between the two arguments
# corr = np.corrcoef(mdata["atac"][:, promoter_gene_pair["peak"].tolist()].X.toarray(), mdata["rna"][:, promoter_gene_pair["nearest_gene"].tolist()].X.toarray(), rowvar=False)
# n_promoter_gene_pairs = promoter_gene_pair.shape[0]
# np.diag(corr[:n_promoter_gene_pairs, n_promoter_gene_pairs:])

atac_x = mdata["atac"][:, promoter_gene_pair["peak"].tolist()].X.toarray()
rna_x = mdata["rna"][:, promoter_gene_pair["nearest_gene"].tolist()].X.toarray()
corr_sc = pearsonr(atac_x, rna_x)

print(f"Mean absolute correlation: {np.abs(corr_sc.statistic).mean():.4f}")
...
#   <blockquote>7.7321</blockquote>
# </div>

# %%
# Implement your solution here
atac_x = pseudobulk_mdata["atac"][:, promoter_gene_pair["peak"].tolist()].X.toarray()
rna_x = pseudobulk_mdata["rna"][:, promoter_gene_pair["nearest_gene"].tolist()].X.toarray()
corr_metacell = pearsonr(atac_x, rna_x)

print(f"Mean absolute correlation: {np.abs(corr_metacell.statistic).mean():.4f}")
print(f"Fold change: {np.abs(corr_metacell.statistic).mean() / np.abs(corr_sc.statistic).mean():.4f}")

# %%
# Fill in the missing code below to plot the single-cell based correlation against the corresponding metacell-based statistic
corr_df = pd.DataFrame({"corr_sc": corr_sc.statistic, "corr_metacell": corr_metacell.statistic})
...
# %%
# Implement your solution here
train_obs_names, test_obs_names = train_test_split(pseudobulk_mdata["atac"].obs_names, test_size=0.25, random_state=0)

# %%
# Implement your solution here
gene_name = "MPO"
row_mask = peak_gene_pairs["nearest_gene"].isin([gene_name])
peak_names = peak_gene_pairs.loc[row_mask, "peak"]

X_train = pseudobulk_mdata["atac"][train_obs_names, peak_names].X.copy()
Y_train = pseudobulk_mdata["rna"][train_obs_names, gene_name].X.copy()

X_test = pseudobulk_mdata["atac"][test_obs_names, peak_names].X.copy()
Y_test = pseudobulk_mdata["rna"][test_obs_names, gene_name].X.copy()
...
# %%
# Implement your solution here
regression_coef = pd.DataFrame(index=peak_names)

train_pred = pd.DataFrame(index=train_obs_names)
train_pred["ground_truth"] = Y_train

test_pred = pd.DataFrame(index=test_obs_names)
test_pred["ground_truth"] = Y_test
...
regression_coef["linear"] = model.coef_.squeeze()
train_pred["linear"] = model.predict(X_train)
test_pred["linear"] = model.predict(X_test)

train_corr = train_pred.corr()
train_corr = train_corr.loc["ground_truth", "linear"]
test_corr = test_pred.corr()
test_corr = test_corr.loc["ground_truth", "linear"]
...
#     <blockquote>The difference in Pearson correlation between ground truth and prediction on train and test sets is the lowest for Ridge regression.</blockquote>
#   <ol type="a">
# </div>
...
print(f"Mean model weight: {model.coef_.mean()}")

regression_coef["ridge"] = model.coef_.squeeze()
train_pred["ridge"] = model.predict(X_train)
test_pred["ridge"] = model.predict(X_test)
...
regression_coef["lasso"] = model.coef_.squeeze()
train_pred["lasso"] = model.predict(X_train)
test_pred["lasso"] = model.predict(X_test)
...
def scan_pwm(sequence: str, pwm: pd.DataFrame, background: dict[str, float]) -> bool:
    ...
    def _get_motif_prob(subseq: str) -> NDArray:
        ...
        """
        return np.diag(pwm[list(subseq)].clip(lower=1e-4))
    ...
    def _get_bg_prob(subseq: str) -> NDArray:
        ...
        """
        return np.clip(list(map(background.get, subseq)), a_min=1e-4, a_max=None)
    ...
    def _get_score(sequence: str) -> NDArray:
        ...
        windows = np.lib.stride_tricks.sliding_window_view(sequence, window_shape=pwm.shape[0])
        motif_prob = np.apply_along_axis(_get_motif_prob, axis=1, arr=windows)
        bg_prob = np.apply_along_axis(_get_bg_prob, axis=1, arr=windows)
        return np.log(motif_prob / bg_prob).sum(axis=1)
...
tf_binding = pseudobulk_mdata["atac"].var.loc[peak_subset, ["sequence"]]
tf_binding["motif_present"] = tf_binding["sequence"].apply(lambda x: scan_pwm(x, pwm=irf8_pwm, background=background))

print(f"{tf_binding['motif_present'].sum() / len(peak_subset) * 100}% of analyzed peaks include the IRF8 motif.")
...
# %%
# Implement your solution here
atac_x = pseudobulk_mdata["atac"][:, tf_binding.index].X.toarray()
rna_x = pseudobulk_mdata["rna"][:, "IRF8"].X.toarray()
corr_res = pearsonr(atac_x, rna_x)
tf_binding["corr"] = corr_res.statistic

# The number of observations is small, so we will not rely on p-values for further analysis
# tf_binding["pval"] = corr_res.pvalue
# tf_binding["padj"] = false_discovery_control(tf_binding["pval"])

print(
    "Number of peaks that include the IRF8 motif and whose correlation between accessibility and IRF8 GEX is at "
    f"least 0.2: {(tf_binding['motif_present'] & (tf_binding['corr'] > 0.2)).sum()}"
)
...
# %%
# Implement your solution here
sc.tl.score_genes(mdata["atac"], tf_binding.index, score_name="background")

row_mask = tf_binding["motif_present"]
sc.tl.score_genes(mdata["atac"], tf_binding.index[row_mask], score_name="unfiltered_irf8_peaks")

row_mask = tf_binding["motif_present"] & (tf_binding["corr"] > 0.2)
sc.tl.score_genes(mdata["atac"], tf_binding.index[row_mask], score_name="filtered_irf8_peaks")
...
#   <blockquote>Yes (p=0.000173).</blockquote>
# </div>
...
# %%
# Implement your solution here
obs_mask = mdata["atac"].obs["celltype"].isin(["Mono", "cDC", "pDC"])
group_1 = mdata["atac"].obs.loc[obs_mask, "filtered_irf8_peaks"].values
group_2 = mdata["atac"].obs.loc[~obs_mask, "filtered_irf8_peaks"].values

# Test if variance in groups is equal
res = levene(group_1, group_2)
if res.pvalue < 0.05:
    equal_var = False
else:
    equal_var = True
res = ttest_ind(group_1, group_2, equal_var=equal_var, alternative="greater")

if res.pvalue >= 0.05:
    print(f"Myeloid cell scores are not significantly higher (p={res.pvalue:.6f}).")
else:
    print(f"Myeloid cell scores are significantly higher (p={res.pvalue:.6f}).")
